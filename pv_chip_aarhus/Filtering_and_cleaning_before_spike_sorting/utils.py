...
def load_data(input_path, dtype=params.dtype, nb_channels=params.nb_channels,
              channel_id=None, probe_size=None, voltage_resolution=params.voltage_resolution, disable=False):
    ...
    if channel_id is None:
            raise ValueError("channel_id must be specified explicitly")  
...
def pair_onsets_offsets(onsets, offsets):
    """
    Pair each onset with the next valid offset.

    Returns:
        pairs (list of tuples): [(onset_idx, offset_idx or None), ...]
    """
    pairs = []
    j = 0
    ...
    for onset in onsets:
        ...
        # Move forward until offset is AFTER onset
        while j < len(offsets) and offsets[j] <= onset:
            j += 1

        if j < len(offsets):
            pairs.append((onset, offsets[j]))
        else:
            # No offset found → open-ended (continuous or truncated)
            pairs.append((onset, None))
...
def write_dead_times_file(triggers_list, onsets, output_directory,
                         exp=params.exp,
                         fs=params.fs,
                         mode_list=None,
                         dead_time_mode='onset'):
    """
    ...
    with open(os.path.join(output_directory, f"{exp}_dead_times.dead"), "w") as f:

        if len(triggers_list) != len(onsets):
            raise ValueError("triggers_list and onsets must have same length")

        for i in range(len(triggers_list)):

            triggers_s = triggers_list[i]
            onset_global = onsets[i]
            mode = mode_list[i] if mode_list is not None else 'poly'

            # --------------------------------------------------
            # Select correct timing (ms → seconds)
            # --------------------------------------------------
            if mode == 'poly':
                t_before = params.poly_time_before / 1000
                t_after  = params.poly_time_after / 1000
            elif mode == 'laser':
                t_before = params.laser_time_before / 1000
                t_after  = params.laser_time_after / 1000
            else:
                t_before = params.poly_time_before / 1000
                t_after  = params.poly_time_after / 1000
            ...
            # CASE 1 — ONSET MODE
            # ==================================================
            if dead_time_mode == 'onset':

                triggers_abs = (triggers_s + onset_global) / fs

                for trig in triggers_abs:
                    start = trig - t_before
                    end   = trig + t_after
                    f.write(f"{start*1000:.6f} {end*1000:.6f}\n")

            # ==================================================
            # CASE 2 — INTERVAL MODE
            # ==================================================
            elif dead_time_mode == 'interval':

                for onset, offset in triggers_s:

                    onset_s = (onset + onset_global) / fs
                    offset_s = (offset + onset_global) / fs if offset is not None else onset_s

                    start = onset_s - t_before
                    end   = offset_s + t_after
            ...
            # ==================================================
                ...
                for onset, offset in triggers_s:
                    ...
                    # onset window
                    f.write(f"{(onset_s - t_before)*1000:.6f} {(onset_s + t_after)*1000:.6f}\n")

                    # offset window
                    if offset is not None:
                        offset_s = (offset + onset_global) / fs
                        f.write(f"{(offset_s - t_before)*1000:.6f} {(offset_s + t_after)*1000:.6f}\n")
